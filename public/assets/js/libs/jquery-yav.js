...
jQuery.fn.extend({
	 ...
	 * @author SeViR Jose Francisco Rives Lirola (http://letmehaveblog.blogspot.com | http://www.sevir.org/en/)
	 */
	yav:function(params,yav_config){
		...
		}, ((typeof(jQuery.yav_default)!="undefined") && (typeof(jQuery.yav_default.library)!="undefined") )?( jQuery.extend(jQuery.yav_default.library, yav_config) ):yav_config);
		...
		var params = jQuery.extend({
			...
			onError : "",
			onOk : ""
		}, (typeof(jQuery.yav_default)=="undefined")?params:( jQuery.extend(jQuery.yav_default, params) ) );
		...
		function setRules(o){
			...
			if (jQuery(o).is("form")){
				...
				for (var name in conds){
					if (conds.hasOwnProperty(name) && conds[name].group == null){
				...
				for(var i=0;i<yav_defRules.length;i++){
					if(jQuery(o).is("."+yav_defRules[i])){
						var o_title = jQuery(o).attr((!params.hideTitle)?"title":params.altTitle);
						if(typeof(o_title) != 'undefined' && o_title.length) {
							o_title = o_title.replace(/'/g,'&apos;');
						}else{
							log(o.id+" without title with rulename "+rulename);
						}

						try{
							str_rule =
								createRule(o,yav_defRules[i],(o_title),((typeof eval("("+jQuery(o).attr("alt")+")").params != "undefined" && typeof eval("("+jQuery(o).attr("alt")+")").params[0] == "object")?eval("("+jQuery(o).attr("alt")+")").params[num_rules]:eval("("+jQuery(o).attr("alt")+")").params),eval("("+jQuery(o).attr("alt")+")").require);
						}catch(e){
							str_rule =
								createRule(o,yav_defRules[i],(o_title));
						}
				...
				for(rulename in jQuery.yav_customfunctions){
					if(jQuery(o).is("."+rulename)){
						...
						try{
							str_rule =
								createRule(o,"jQuery.yav_customfunctions."+rulename,(o_title),(typeof  eval("("+jQuery(o).attr("alt")+")").params !='undefined')?((typeof eval("("+jQuery(o).attr("alt")+")").params[0] == "object")?eval("("+jQuery(o).attr("alt")+")").params[num_rules]:eval("("+jQuery(o).attr("alt")+")").params):'',eval("("+jQuery(o).attr("alt")+")").require);
						}catch(e){
							str_rule =
								createRule(o,"jQuery.yav_customfunctions."+rulename,(o_title));
						}
						arr_rules.push(str_rule);
						num_rules++;
					}
		...
		function createRule(o, rulename, text, rule_params, require){
			...
			if (rulename.indexOf("jQuery.yav") >= 0){
				return rulename+"({id:'" + o.id + "',msg:'" + text + "'"+((typeof(require) == "undefined")?"":",require:'"+require+"'") +"}"+
						...
						strParams(rule_params, ",") +")|custom"+((typeof require=='undefined')?'':'|'+require);
			}else{
		...
		if (typeof(jQuery.yav_customfunctions) == "undefined"){
			jQuery.yav_customfunctions = params.custom;
		}else{
			jQuery.yav_customfunctions = jQuery.extend( jQuery.yav_customfunctions, params.custom );
		}
		...
		return this.each(function(){
			if (params.hideTitle){
				$('input, textarea, select',this).each(function(){
					$(this).attr(params.altTitle, $(this).attr("title")); //set alternative title
					$(this).removeAttr('title'); // remove the title
				});
			}

			if(!params.rulesOnSubmit){
				this.formRules = setRules(this);
			}
