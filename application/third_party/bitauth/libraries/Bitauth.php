...
class Bitauth
{
	...
	public function __construct()
	{
		...
		$this->_table_ids					= $this->config->item('table_ids', 'bitauth');
	...
	public function update_remember_token($username = NULL, $user_id = NULL)
	{
		...
		if($user_id === NULL)
		{
			$user_id_field = $this->_table_ids['users'];
			$user_id = $this->$user_id_field;
		}
	...
	public function add_user($data, $require_activation = NULL)
	{
		...
		// Just in case
		if( ! empty($data[$this->_table_ids['users']]))
		{
			unset($data[$this->_table_ids['users']]);
		}
	...
	public function add_group($data)
	{
		...
		// Just in case
		if( ! empty($data[$this->_table_ids['groups']]))
		{
			unset($data[$this->_table_ids['groups']]);
		}
	...
	public function update_user($id, $data)
	{
		...
		if( ! empty($data))
		{
			$this->db->set($data)->where($this->_table_ids['users'], $id)->update($this->_table['users']);
		}
	...
	public function update_group($id, $data)
	{
		...
		// Just in case
		unset($data[$this->_table_ids['groups']], $data['id']);
	...
	public function username_is_unique($username, $exclude_user = FALSE)
	{
		...
		{
			$this->db->where($this->_table_ids['users'].' !=', (int)$exclude_user);
		}
	...
	public function group_is_unique($group_name, $exclude_group = FALSE)
	{
		...
		{
			$this->db->where($this->_table_ids['groups'].' !=', (int)$exclude_group);
		}
	...
	public function get_users($include_disabled = FALSE)
	{
		...
		$query = $this->db
			...
			->join($this->_table['data'].' userdata', 'userdata.user_id = users.'.$this->_table_ids['users'], 'left')
			->join($this->_table['assoc'].' assoc', 'assoc.user_id = users.'.$this->_table_ids['users'], 'left')
			->join($this->_table['groups'].' groups', 'groups.'.$this->_table_ids['groups'].' = assoc.group_id', 'left')
			->group_by('users.'.$this->_table_ids['users'])
	...
	{
		$this->db->where('users.'.$this->_table_ids['users'], $id);
		$users = $this->get_users($include_disabled);
	...
	public function get_groups()
	{
		$query = $this->db
			...
			->join($this->_table['assoc'].' assoc', 'assoc.group_id = groups.'.$this->_table_ids['groups'], 'left')
			->group_by('groups.'.$this->_table_ids['groups'])
	...
	{
		$this->db->where('groups.'.$this->_table_ids['groups'], $id);
		$groups = $this->get_groups();
