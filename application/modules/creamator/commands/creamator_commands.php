...
$creamator['basicCommands'] = array(
	...
	'updateFastAutoload' => array(
		'description'=>'Update file of fast autoloading classes',
		'usage' => 'creamator updateFastAutoload',
		'num_parameters' => 0
	),
	'fastcacheClear' => array(
		'description'=>'Clears cache of Fastcache',
		'usage' => 'creamator fastcacheClear',
		'num_parameters' => 0
	),
	'server'=>array(
		'description'=>'Run internal PHP Web Server',
		'usage' => 'creamator server <localhost:port>',
		'parameters'=> array(_('Server uri and port separated with : [optional]')),
		'num_parameters' => 0
	)
);
...
function updateFastAutoload(){
	...
	//standard paths
	$enabled_paths = array(realpath(BASEPATH), realpath(CREAMTURE_PATH.DIRECTORY_SEPARATOR),realpath(APPPATH));
	//add packages paths
	foreach ($autoload['packages'] as $package) {
		array_push($enabled_paths, realpath($package.DIRECTORY_SEPARATOR));
	}
	$enabled_paths = array_unique($enabled_paths);

	foreach ($enabled_paths as $path){
		$list = directory_map($path,2);
		foreach (array('core','libraries','helpers','views') as $folder) {
			if (isset( $list[ $folder ] )){
				foreach ($list[ $folder ] as $f) {
					if ( strpos( $f, '.php') > 0 ){
						$class = str_replace('.php', '', $f);
						$autoload_cache[ $class ] = $path.DIRECTORY_SEPARATOR.$folder.DIRECTORY_SEPARATOR.$f;
					}
				}
			}
		}
	...
	$str_file = '<?php '."\n";
	foreach ($autoload_cache as $c=>$p) {
		$str_file .= '$a[\''.$c.'\'] = \''.$p.'\';'."\n";
	}
	file_put_contents(APPPATH.'/cache/libcache.php', $str_file);

	println('libcache created!');
}

function fastcacheClear(){
	$CI = & get_instance();
	$CI->load->library('FastCache');
	$CI->fastcache->clean();

	println('FastCache cleaned');
}

function server($serverDomainAndPost='localhost:8000'){
	$php_version = phpversion();
	if ( version_compare($php_version, '5.4', '>=')){
		println('Running internal server in '.$serverDomainAndPost.' ...');
		println('CTRL+C to exit');

		$output = shell_exec('php -S '.$serverDomainAndPost.' -t public');	
		println($output);
	}else{
		println('This command need of PHP 5.4 or below');
	}	
}
